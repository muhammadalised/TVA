...
class EdSmokeConfigTests(unittest.TestCase):
    @classmethod
    ...
    def test_all_four_smoke_conditions_are_strictly_matched(self):
        ...
        for name, config in self.configs.items():
            with self.subTest(config=name):
                ...
                self.assertEqual(
                    config['ed_distribution'], expected_distribution
                )
...
class EdProductionConfigTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.configs = {
            name: yaml.safe_load(path.read_text(encoding='utf-8'))
...
class EdZipDatasetTests(unittest.TestCase):
    def setUp(self):
        if not all(
            (ED_DIRECTORY / f'gold_{distribution}.zip').exists()
            for distribution in ('wd', 'wi')
        ):
    ...
    def test_environment_override_and_fold_metadata(self):
        with patch.dict(os.environ, {'TVA_ED_DATASET_DIR': str(ED_DIRECTORY)}):
            self.assertEqual(get_ed_num_folds('unused', 'wd'), 5)
            self.assertEqual(get_ed_num_folds('unused', 'wi'), 5)
    ...
    def test_loader_reads_and_collates_seven_channel_samples(self):
        ...
        self.assertIsInstance(dataset, EdZipDataset)
